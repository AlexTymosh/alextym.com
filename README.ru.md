...
<p align="center">
  <em>Portfolio-сайт, сделанный как небольшой AI-powered web product: интерактивное резюме, RAG-чат, SEO-настройка, video demo, contact flow и human handoff через Telegram.</em>
</p>
...
На сайте есть AI-чат, интерактивное резюме, contact form, YouTube demo и механизм **human handoff**: если AI не справляется с ответом или посетитель просит прямого контакта, чат предлагает переключение на владельца сайта. В таком сценарии Telegram webhook передаёт активную сессию с историей, а состояние чата переключается между AI-ассистентом и человеком.

RAG-пайплайн использует Qdrant + OpenAI embeddings. Для повышения качества ответов совокупно используются следующие подходы:
- dense vector search
...
- `keywords_sparse` как keyword channel для retrieval hints / scoring, - это не полноценный Qdrant sparse-vector index, а практичный псевдо-sparse слой поверх metadata и keyword scoring для малых документов.
...
- Чат реализован как **hybrid chat interface**: scripted bot для быстрых сценариев, AI/RAG для ответов по публичной базе знаний и human handoff для перехода к владельцу сайта. Данная комбиинация позволяет дать быстрые ответы, структурированный AI-поток, имеет fallback-логика и эскалацию вместо попытки заставить AI отвечать на всё.
- AI-ассистент отвечает на вопросы о публичном профессиональном профиле владельца сайта.
- Чат поддерживает streaming-ответы через Server-Sent Events.
- Если streaming недоступен, используется JSON fallback endpoint.
- Ответ возвращается в структурированном формате: `answer`, `sources`, `confidence`, `not_enough_data`, `handoff_suggested`, `handoff_reason`.
- Короткая история диалога используется для follow-up вопросов и pronoun resolution, но не является источником фактов.
- AI ассистент настроен на минимизацию галюцинаций и в случае нехватки данных должен честно сообщить о недостатке информации и предложить human handoff для уточнения вопроса.
...
Если вопрос требует прямого контакта, уточнения доступности, hiring/collaboration discussion или AI не может дать надёжный ответ, ассистент должен предложить подключить владельца сайта. При согласии посетителя:
...
- динамическая download-ссылка для CV: PDF формируется с учётом выбранного уровня детализации и выбранных секций.

Это сильнее обычного PDF: работодатель может быстро посмотреть короткую версию, а затем открыть более подробную.
...
- Есть honeypot-поле `company_website` против простых ботов: если honeypot заполнен, backend не применяет обычный путь обработки как к реальному сообщению.

### 🔎 SEO / SMM readiness

Сайт настроен как публичная страница, которую можно индексировать и показывать рекрутёрам/заказчикам/партнерам:

- centralized `siteConfig` для title, description, keywords и публичных routes;
- page metadata для главной страницы, chat page и resume page;
- canonical URLs;
- OpenGraph metadata для LinkedIn / social previews;
- Twitter card metadata;
- JSON-LD `Person` structured data;
- `sitemap.xml` для публичных маршрутов;
- `robots.txt` с запретом индексации `/api/`;
- preview deployments могут быть закрыты от индексации;
- favicon и OpenGraph image подключены как часть публичной презентации сайта.

> Это не “магическое SEO”, а базовая техническая подготовка сайта к корректной индексации и социальным preview. 
> Если вы форкните данный проект, не забудьте исправить SEO оптимизацию под свои нужды
...
Метрики из JSON-отчётов на момент публикации:
...
> Cайт можно улучшить — мелкие accessibility issues с контрастом accent/button, чтобы приблизить Accessibility к 100 (P4 приоритет).
...
- scope routing: чат отвечает только на вопросы о публичном профессиональном профиле владельца сайта;
- private-data boundary: phone, personal email, home address и приватные сведения не содержаться и не раскрываются;
- no-hallucination policy: если контекста недостаточно, возвращается insufficient-data response;
...
- Eval reports можно сравнивать в формате before/after, чтобы видеть regressions/fixes после изменения знаний, prompt или retrieval logic.

---

## 💬 Как работает чат

Чат построен как гибридный слой коммуникации:

```text
scripted responses
  -> быстрые ответы для типовых recruiter-facing сценариев
AI/RAG responses
...
`/api/warmup` зависит от времени простоя backend: он полезен после idle period / cold start на free или low-cost hosting, где backend может засыпать. Если backend уже активен, этот шаг не должен восприниматься как отдельная пользовательская функция — это технический warm-up.

> Backend данного проекта для минимизации затрат размещён на Render Free Tier. Бесплатный тариф усыпляет сервис после неактивности. Для удобства пользователей рекомендуется настроить cron-job или uptime monitor с регулярным вызовом health/warmup endpoint каждые 10–15 минут. 
...
Это важный production-style элемент: интерфейс не зависит только от streaming channel.
...
После предложения handoff пользователь не обязан переходить к разработчику: он может отказаться и продолжить обычный AI/chat flow. В активной handoff-сессии пользователь также может закрыть связь с разработчиком и вернуться к обычному чату.
...
| **Query routing** | Вопрос классифицируется по intent: skills, projects, availability, right_to_work, experience, education, contact и др. |
| **Query rewriting / subject resolution** | Короткие follow-up вопросы и pronouns переписываются в standalone Alex-focused retrieval query. |
| **Query expansion** | Для тем вроде FastAPI, SQL, RAG, projects, experience добавляются дополнительные retrieval terms. |
...
|---|---|---|
| `GET` | `/api/health/live` | дешёвая проверка, что backend жив |
...
| `POST` | `/api/chat/stream` | streaming chat через SSE |
...
- `/api/health/live` — дешёвый liveness endpoint;
