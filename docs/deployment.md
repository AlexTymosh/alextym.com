...
The source code confirms `/api/*` frontend calls and a `BACKEND_ORIGIN`-driven
rewrite to the backend. DNS provider, hosting tier, Qdrant tier, backend host,
and keep-alive service are deployment choices outside the application code and
must be verified in provider dashboards.
...
The setup wizard edits only committed non-secret project config. It does not set
Vercel, Render, Qdrant, Resend, Telegram, Upstash, or OpenAI secrets. Configure
those values in provider dashboards. The wizard runs only on `127.0.0.1`, groups
settings by tab, requires `Review & Save` validation before writing changes, and
can be stopped with `Finish setup` after setup.
It exposes reusable public template values only. Navigation routes, footer
boilerplate, canonical page paths, Open Graph image dimensions, and standard
chat handoff/status messages are generated by the application.
Language restriction toggles, read-only system fallback previews, quick
prompts, resume PDF link visibility, and the full disclaimer body are handled
through the local wizard before deployment. Resume profile text is derived from
...
```text
frontend/next.config.mjs reads BACKEND_ORIGIN
/api/:path* -> ${BACKEND_ORIGIN}/api/:path*
```

`frontend/vercel.json` is intentionally not used for backend rewrites. It must
not contain a hardcoded backend deployment host.

Expected `frontend/vercel.json` shape:
...
- backend hosting can be changed by updating `BACKEND_ORIGIN` in Vercel and
...
This check fails if rewrites are reintroduced into `frontend/vercel.json`, or if
Vercel builds would not fail fast when `BACKEND_ORIGIN` is missing.

Safe rollout order for an existing live site:

1. Deploy or verify the backend first.
2. Confirm the backend health endpoint works directly.
3. Set `BACKEND_ORIGIN` in Vercel for the relevant environments.
4. Redeploy the frontend with the environment variable present.
5. Verify `/api/health/live`, `/api/health/ready`, and `/api/warmup` through the
   frontend domain.
...
- update the Vercel `BACKEND_ORIGIN` frontend environment variable;
- redeploy frontend;
...
- [ ] Set `BACKEND_ORIGIN` to the backend origin, for example
      `https://<backend-host>`.
